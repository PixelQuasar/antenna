...
mod test {
    ...
    fn joiner_available_after_bootstrap_and_after_relay() {
        ...
        assert!(charlie.is_available(), "charlie should be available immediately after bootstrap");
        ...
        assert!(peers[&charlie_id].is_available(), "charlie should be available after relay with alice completes");
    }
    ...
    fn joiner_waits_for_all_relays() {
        ...
        // Dave joins via Alice (who has 2 existing connections: Bob and Charlie)
        ...
        assert!(dave.is_available(), "dave should be available immediately after bootstrap");
        ...
        establish_relay_connection(&mut peers, &alice_id, &dave_id, &appeared[0], &relay_messages);
        assert!(peers[&dave_id].is_available(), "dave should be available after first relay completes");

        establish_relay_connection(&mut peers, &alice_id, &dave_id, &appeared[1], &relay_messages);
        assert!(peers[&dave_id].is_available(), "dave should be available after all relays complete");
    }
}
